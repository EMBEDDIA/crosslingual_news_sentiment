...
def crossvalidation_sampling():
    ...
    output_dir = os.path.join(args.output_dir, "sampling")
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    log_path = os.path.join(output_dir, "log2")
    ...
    for i in range(9, 10):
        ...
        classification_train_dataloader = prepare_classification_head_dataset(train_data, train_labels, model,
                                                                              tokenizer, device, args.max_len,
                                                                              args.batch_size)
        classification_eval_dataloader = prepare_classification_head_dataset(eval_data, eval_labels, model,
                                                                             tokenizer, device, args.max_len,
                                                                             args.batch_size)
        ...
        test_dataloader = prepare_classification_head_dataset(test_data, test_labels, model, tokenizer, device,
        ...
        cro_test_dataloader = prepare_classification_head_dataset(cro_test_data, cro_test_labels, model, tokenizer,
