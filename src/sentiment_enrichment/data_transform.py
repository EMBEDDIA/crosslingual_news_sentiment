...
def prepare_data_for_language_modelling(data, labels, tokenizer, max_len, batch_size):
    ...
    masked_sentences_with_special_tokens = [["[CLS]"] + sentence + ["[SEP]"] for sentence in masked_sentences]
    # adding ids for [CLS] and [SEP] special tokens, to be ignored by the loss function
    final_mlm_labels = []
